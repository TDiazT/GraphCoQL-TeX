...
\small
    ...
    % \td{Is it better?}\td{The functions that are not named are the same as the ones in the semantics -- should we redefine them?}
    	\newline{\footnotesize 
...
Secondly, we realized that using \HP's equivalence rules (\cf \S3~\cite{gqlph}), some queries cannot be normalized\td{I just realized that the version of the paper we are citing does not include the equivalence rules}. For instance:
\begin{minted}[escapeinside=||, mathescape=true]{js}
          query {
              name
              |$\ldots$| on Query {
