...
\noindent The left query is not grounded because the field \texttt{title} is directly selected on the abstract type \texttt{Movie} type. In contrast, the right query is grounded because it specifies the (concrete) object types \texttt{Film} and \texttt{Animation} on which it requests the information.
...
\texttt{title} twice to the same type, and it uses two fragments with
...
Since the type of the field \texttt{movie} is the abstract type \texttt{Movie}, the subselections are wrapped in fragments for each concrete object subtype, namely \texttt{Fiction} and \texttt{Animation}. The normalized query is both non-redundant and in ground-typed normal form. 
...
\begin{figure*}[t]
  ...
  \centering
    ...
    \caption{Simplified semantics for selections in normal form,
      adapted from~\cite{gqlph}.\fo{flush left}} 
