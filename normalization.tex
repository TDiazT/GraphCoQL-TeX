...
\begin{itemize}
    \item There is at most one field selection with a given response name, for a particular depth of the selection set.\td{Depth?}\et{define depth and size for selection sets in section 3}
...
\small
    ...
    \end{cases}
    \end{align*}
    \caption{Normalization procedure for \gql selections. \td{Is it better?}\td{The functions that are not named are the same as the ones in the semantics -- should we redefine them?}
    	\newline{\footnotesize The predicate $\mathit{is\_object\_type}$ checks whether the given type is defined as an object type in the schema. 
	Meanwhile, the function $\mathit{get\_possible\_types}$ returns all object subtypes of the given type.}
    }
\label{fig:normalize}
\end{figure*}
...
\begin{figure*}[t]
    ...
    \end{cases}
    \end{align*}
    \caption{Simplified semantics for selections in normal form, adapted from~\cite{gqlph}. \td{Fixing}} %\et{why is it $\in L$ and not $= L$?}}
