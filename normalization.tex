...
This definition of groundedness differs slightly from that of \HP{} \et{how is it different? why?}\td{See response in Zulip}, nevertheless, we prove that our definition still implies being in ground-typed normal form \et{if you prove that it means you have also formalized the exact def from HP? so?}.
...
%As described initially in this paper \et{you recalled it in the header of 4, no need to say it again}, \HP{} base the complexity results over GraphQL queries on two premises; queries can be normalized, preserving their semantics, and there is an equivalent simplified function to evaluate queries in normal form. 
We prove the premise that any query can be normalized and still preserve its semantics, by establishing correctness of our procedure and then proving the latter. 
