...
\begin{wrapfigure}[9]{r}{.25\textwidth}
\vspace{-2.5ex}
...
\texttt{Dog} type, while in the second it is requesting information about the \texttt{Pig} type. The distinction is important because some field selections might be valid in some contexts but not in others. 
\td{Better? I feel there is something wrong, but not sure what} This is the case, for instance, for field \texttt{favoriteToy}: It is valid in the scope of a \texttt{Dog} type but it is invalid in the scope of a \texttt{Pig} type, as \texttt{Pig} type does not contain any such field. %Similarly, the above query would also be invalid if \texttt{Pig} type contained a so-called field, but of type different from \texttt{Toy} ---the type of the same field in type \texttt{Dog}.
...
\centering
 ...
 %    & \\[2ex]
    ...
    % SingleField
    & (2) & \evalu{\fld :: \sset} &~=~ 
        \resp{(\mathit{complete\_value(ftype(type(u), \texttt{f}), property(u, \fld))})} :: \evalfilteru{\sset}{\fkey}  \\
    % Nested field
    & (3) & \evalu{\nfld{\overline{\beta}} :: \sset} &~=~
    \begin{cases}
        \resp{\texttt{[} \mathit{map}\; (\lambda v_{i}.\; \eval{\overline{\beta} \mdoubleplus \mathit{merge (collect(type(u), \fkey, \sset))}}{v_{i}})\; \mathit{neighbors}_{\graph}(u, \fld) \texttt{]}} :: \evalfilteru{\sset}{\fkey} \\  
         \hfill \text{ if } 
  ...
  \text{ if } \mathit{ftype_{type(u)}(\fkey)}  \neq \mathit{list}  \text{ and } \nexists v \text{ s.t. }  (u, \fkey[\alpha], v) \in \mathit{edges}(\graph) \\
    ...
    \evalu{\overline{\beta} \mdoubleplus \sset} 
     & \text{ if }\mathit{fragment\_type\_applies(type(u), \texttt{t})}\\
    \evalu{\sset} 
    & \text{ otherwise}
    \end{cases}
    \end{align*}
    ...
    \td{To make it look cleaner, we can maybe use "u.property($\fld$)" and "u.type"}
    \td{Not forget to update the simplified semantics}
    $\mathit{edges}$ gets the set of edges of a graph.  $\mathit{is\_valid\_response}$ validates the coerced value.
    $\mathit{ftype}$ retrieves the type of a field in a schema. $\mathit{list}$ represents the list type.}}
...
Roughly speaking, a \gql response maps keys (fields names together
with their arguments)\td{?}\td{a response maps keys to values, not their arguments. Or you meant the semantics?} to response
...
\begin{definition}
  ...
  \mathit{Scalar} \to \mathit{Bool}$. \td{Mention that $\schema$ is also an implicit param}
\end{definition}
...
does not have type list, then there should be at most one neighboring
node; if there is no neighbor,
the result is considered \texttt{null}. \td{Rewrote this to use "at most" and simplify text}
