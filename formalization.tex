...
%
\td{Should include Variable (Val : eqType) here as well?}
\begin{minted}[bgcolor=coqbg]{coq}
Record wfGraphQLSchema := WFGraphQLSchema {
    schema : graphQLSchema;
    _ : schema.(is_a_wf_schema);
    is_a_valid_value : type -> Val -> bool; }.
\end{minted}
...
\begin{minted}[bgcolor=coqbg]{coq}
Record label := 
  Label { lname : string; args : seq (string * Val) }.

Record node := 
  Node { ntype : Name; nprops : seq (label * Val) }.
...
We model this with a function  $\mathit{coerce} : \mathit{Val} \to \mathit{ResponseVal}$.
