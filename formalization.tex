...
%
\td{The code actually looks like that, with the @ in every Value occurrence. Should we leave it or remove it for presentation?}
\begin{minted}[bgcolor=coqbg]{coq}
Record label := 
...
\td{I am trying to push this definition here and then not having to redefine it or explain it again}
...
\td{Since I mention above that it uses this predicate and show it as a variable, maybe we can skip it?}
\begin{minted}[bgcolor=coqbg]{coq}
...
We model this with a function  $\mathit{coerce} : \mathit{Value} \to \mathit{ResponseValue}$.
% \td{Does this make more sense with the changes?}
...
%\et{so fix the signature!}
%
%\et{Also, you have both "Vals" and "Value" as names/postfixes for something that should be essentially the same.}
%
\td{We build a wfCoercion by proving that the coerced value is not redundant -- this might need more explanation}.\et{where is that used/relevant? I see that wfCOercion only shows up, without explanation, in Section 4. It should be introduced in section 3, maybe later below (it's not necessary for the figure it seems)}\et{when showing execute-query! btw your signature here does not match the one you use in section 4, which has a wfCoercion as argument! (beware with those details)}
\td{Yeah, you are right -- I'll change it}
...
This is performed by calling the $\mathit{collect}$, $\mathit{merge}$ and $\mathit{filter}$ functions, which respectively gather fields sharing the same response name, merge their subselections, and remove their repeated occurrences. Finally, inline fragments (4) simply condition whether their subselections should be evaluated in the current node. The decision is based on the $\mathit{fragment\_type\_applies}$ predicate that verifies whether the type condition is the same or a supertype of the node's type.\footnote{Aliased fields are not included here; they are evaluated the same way as unaliased fields, but differ in that the key-value pair produced is renamed accordingly.}
...
Definition execute_query (s : wfGraphQLSchema)
  ...
  (|$\varphi$| : query) :=
    execute_selection_set s g g.(root) |$\varphi$|.(selection_set).
\end{minted}
