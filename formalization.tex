...
\small
    ...
    \end{cases}
    \end{align*}
    \caption{Semantics of \gql queries\et{(more precisely, selection sets) -- in fact, should you still use $\varphi$ here, for selections, since you use it in the text for queries? the text now uses $ss$ for selection set, which does not lend itself well to this overbar representation. Needs to be fixed}.\newline {\footnotesize The $\mathit{property}$ function is an accessor to a node's property value. The $\mathit{filter}$ function removes all fields with the given response name, while $\mathit{collect}$ gathers all fields that match the given response name. 
