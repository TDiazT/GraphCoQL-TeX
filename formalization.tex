...
%    \alt \textbf{\texttt{type}} <name> \textbf{\texttt{\{}} <Field>$^{+}$ \textbf{\texttt{\}}}
    ...
    \end{subfigure}
    \caption{Definition of \gql types, the \spec's grammar (left) and \coq formalization (right). Square brackets represent optional values. \fo{In the grammar, I would push the scalar case to a new line, to match the \coq snippet on the right}}
    \label{fig:types_def}
\end{figure*}
...
\begin{itemize}
    ...
    \item a collection of edges of the form ($u$,
      \texttt{f[}$\alpha$\texttt{]}, $v$), where $u, v$ are nodes and
      \texttt{f[}$\alpha$\texttt{]} is a label with arguments
      (key-value pairs).  \fo{Since a label can
        have multiple arguments, using $\overline{\alpha}$ is
        preferable to $\alpha$.}
\end{itemize}
\end{definition}
...
\end{minted}
\td{Should rename E to edges, so it is clearer?}\fo{Yes. Also push it
  to a newline, to avoid the column overflow.}
%
...
graphs is fully independent of schemas. To relate the data to the type system, we define the notion of \textit{conformance} of a graph \wrt a schema.
%
\begin{definition}
...
\fo{We should mention this also in \S 2} and a list of selections. A selection can be a single field, a field followed by a set of subselections or an inline fragment comprising a type condition and a set of subselections. Fields can be accompanied by a list of arguments and can also be renamed.
...
\begin{figure*}[h]
  ...
  \begin{displaymath}
	\begin{array}{rcl}
	\query & ::= & \textbf{\texttt{query \{}} \sset \textbf{\texttt{\}}} \\
	&&\\
	\sel & ::= & \fld \\
	& | & \afld \\		
	& | & \nfld{\sset} \\
  ...
  \begin{subfigure}{.5\textwidth}
    ...
    \end{minted}
  \end{subfigure}
  \caption{Definition of \gql queries, the \spec's grammar to the left (\cf\S2.3~\cite{gqlspec}) and the \coq implementation (right).  
  \newline{\footnotesize The keywords \texttt{f}, \texttt{a} and \texttt{t} correspond to identifiers used for field names, field aliases and type conditions respectively.
  Meanwhile, $\alpha$ is a list of key-value pairs, representing the arguments of fields.}
...
\noindent In one case, the field is requesting information about a
\texttt{Dog} type, while in the other case  it is requesting information about the \texttt{Pig} type. (These are the respective types in context for the first and second occurrence of the field.)\td{?} The distinction is important because some field selections might be valid in some contexts but not in others. This is the case, for instance, for field \texttt{favoriteToy}: It is valid in the context of a \texttt{Dog} type but it is invalid in the context of a \texttt{Pig} type, as \texttt{Pig} type does not contain any such field. %Similarly, the above query would also be invalid if \texttt{Pig} type contained a so-called field, but of type different from \texttt{Toy} ---the type of the same field in type \texttt{Dog}.
...
\begin{itemize}
    \item every selection in $\sset$ is consistent, 
...
In \gcoql, this is captured as with the following predicates.
...
Definition selections_conform (s : wfGraphQLSchema)
    (ts : Name) (@$\sel$@ : seq Selection) : bool :=
        all (is_consistent ts) @$\sel$@ &&
        is_field_merging_possible ts @$\sel$@ &&
...
The definition starts with the base case of empty selection set, which results in an empty response. 
Fields without subselections (equation 2) correspond to accessing a node's property that matches the field name. 
...
We model this with the function $coerce : Vals \to ResponseValue$\td{We build a wfCoercion by proving that the coerced value is not redundant -- this might need more explanation}.
The case of fields with subselections (equation 3) represent a traversal to neighboring edges. The semantics searches for nodes that are connected by an edge whose label matches the field
...
The $\mathit{fragment\_type\_applies}$ predicate verifies that the type condition is related\td{share at least one subtype} to the node's type and if the fragment should be evaluated.\footnote{Aliased fields are not included since they are evaluated the same way as unaliased fields, but differ in that the key-value pair produced is renamed.}
...
\small
    ...
    \begin{cases}
        ...
        \hfill \text{ if } 
            \mathit{ftype}_{\mathit{type}(u)}(\fkey) \text{ is } L_{t} \text{ and } \{v_{1}, \ldots, v_{k}\} = 
        \{v_{i} \mid (u, \fkey[\alpha], v_{i}) \in \mathit{edges}(\graph)\} \\
        %
    (\fkey:\{\eval{\subqueries{\beta}}{v}\}) ::  \evalfilteru{\sset}{\fkey}  
        \hfill \text{ if } 
        \mathit{ftype}_{\mathit{type}(u)}(\fkey)  \text{ is not } L_{t} \text{ and } (u, \fkey[\alpha], v) \in \mathit{edges}(\graph) \\
    %
    \resp{\nval} :: \evalfilteru{\sset}{\fkey} 
    \qquad \qquad \qquad \qquad \qquad \qquad \qquad \qquad
    \text{ if } \mathit{ftype}_{\mathit{type}(u)}(\fkey)  \text{ is not } L_{t}  \text{ and } \nexists v \text{ s.t. }  (u, \fkey[\alpha], v) \in \mathit{edges}(\graph) \\
    \end{cases}\\
    %inline fragment
    & (4) & \evalu{\ifrag{t}{\overline{\beta}} :: \sset} &= \begin{cases}
    \evalu{\overline{\beta} \mdoubleplus \sset} 
    & \text{ if }\mathit{fragment\_type\_applies}_{\mathit{type}(u)}(t)\\
    \evalu{\sset} 
    & \text{ otherwise}
    \end{cases}
    \end{align*}
    \caption{Semantics of \gql selection sets. 
    \newline {\footnotesize 
    The $\mathit{property}$ and $\mathit{type}$ functions are accusers to a node's property and type, respectively. 
    Meanwhile, the functions $\mathit{edges}$ and  $\mathit{neighbors}$ correspondently get the set of edges of the graph 
    and the neighboring nodes, whose edge is labeled with the given field.
    Finally, $\mathit{is\_valid\_response}$ validates the coerced value, $\mathit{ftype}$ retrieves the field's type in the given type in the schema, and $L_{t}$ represents the list type.
	...
	\et{then why the $_t$ subscript?}\td{Simply bc its "List type" -- I took the same notation as in \HP to somehow show the similarity between both definitions.}}
...
and \gcoql have a slightly limited graph model that does not entirely describe the universe of a \gql service: graphs cannot model {\em nested} lists, when the underlying type is an object or abstract type.\footnote{The situation is worse in \HP, which cannot even model nested list types for scalar types.}
