...
dataset. A \texttt{Name} is simply a string but renamed to match the \spec.
...
% enforces this
% requirement , we perform this validation when verifying
% the (overall) well-formedness of schemas.     
% \et{fix}For the corresponding type definitions to be valid, all
% such three sets should be non-empty. While 
...
($\ifrag{t}{\sset}$). Fields can be aliased ($\afld$, $\afld \texttt{\{} \sset \texttt{\}}$). For
...
%  think it's good}
For instance, this is the case of field \texttt{favoriteToy}, which is valid in the scope of the \texttt{Dog} type but it is invalid in the scope of the \texttt{Pig} type, as the \texttt{Pig} type does not contain any such field. %Similarly, the above query would also be invalid if \texttt{Pig} type contained a so-called field, but of type different from \texttt{Toy} ---the type of the same field in type \texttt{Dog}.
...
\wrt the type in scope. This requirement depends on to the
...
% can be traced to reference implementations, and 
...
% at this stage of development. %compared to the simple reasoning principle that trees enjoy.\et{is it?}
