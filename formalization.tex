...
\td{We are not exactly capturing that the fields can be inside fragments, is it ok?}
...
Finally, most of the recursive definitions over selection sets are well-founded recursion over a notion of size.
\begin{definition}[adapted from \cite{gqlph}]\td{It is in essence the same, except that they define selections along with queries}
