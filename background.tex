...
%the reminder of the presentation.
...
the dataset.  %, and discusses the definition of an API to access
%the dataset.
...
either requesting a particular animal or by searching for an object in the union type \texttt{Search}.
...
describe the object content. Edges between nodes are accompanied with
a label that indicates the relationship they establish. For instance,
the top-most and right-most nodes in the graph represent two objects
of type \texttt{Dog} and \texttt{Toy}, respectively. To indicate that
they are related by the \texttt{favoriteToy} relation, the graph
includes a so-labelled edge between these nodes. This is possible
because objects of type \texttt{Dog} contain a field
\texttt{favoriteToy}, holding a value of type \texttt{Toy}.
...
That is the case of the selections \ifrag{Dog}{ favoriteToy \{ chewiness \} }, which indicates that if the friend is a dog then it should request information on its favorite toy, and 
\ifrag{Pig}{ loudness:oink }, specifying that for a pig friend it should request the field \texttt{oink}. The field selection \texttt{loudness:oink} indicates that the resulting value should be renamed 
to \texttt{loudness}, instead of \texttt{oink}.

This query is then evaluated over the graph, resulting in the response depicted to the right in Figure~\ref{fig:qres_ex}, which closely resembles the query.
