...
Marie and is 4 years old. The node outgoing edges say that the dog's
favorite toy is represented by the graph rightmost node and that the dog's
(only) friend is represented by the graph central node.
...
\ifrag{Dog}{\texttt{favoriteToy \{ chewiness \} }} and \ifrag{Pig}{
  \texttt{loudness:oink} }, which indicates that in the case of a 
...
field name \texttt{oink}, in the response the resulting value should
be renamed to \texttt{loudness}. %\fo{Maybe introduce here ``inline
